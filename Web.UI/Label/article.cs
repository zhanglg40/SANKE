...
namespace Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        //获取商品总列表
        protected DataTable view_goods( int page_index,out int totalcount, out string pagelist)
        {
            DataTable dt = new DataTable();
            int pagesize=30;
            totalcount = 0;
            pagelist = "";
               dt = new BLL.article().GetListgood(page_index, out totalcount,  pagesize).Tables[0];
              pagelist = Utils.OutPageList(pagesize, page_index, totalcount, "index.aspx?page=__id__", 8);
        ...
        /// <returns>DataTable</returns>
        protected DataTable get_article_listpp(string channel_name, int category_id, int vote_id, int page_index, string strwhere, out int totalcount, out string pagelist, string _key, params object[] _params)
        {
            DataTable dt = new DataTable();
            int pagesize;
            if (!string.IsNullOrEmpty(channel_name))
            {
                dt = new BLL.article().GetListpp(channel_name, category_id, vote_id, page_index, strwhere, "Barcode asc", out totalcount, out pagesize).Tables[0];
