    ...
    public class download : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (model.point > 0 )
            {
                ...
                //防止重复扣积分
                string cookie = Utils.GetCookie(AXKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString());
                if (cookie != model.id.ToString())
                {
                    ...
                    //写入Cookie
                    Utils.WriteCookie(AXKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString(), model.id.ToString(), 8640);
                }
            }
            //下载次数+1
