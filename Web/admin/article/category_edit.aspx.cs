...
namespace Web.admin.article
{
    public partial class category_edit : Web.UI.ManagePage
    {
        ...
        private void ShowInfo(int _id)
        {
            ...
            txtclassno.Text = model.class_no.ToString();
            txtType.Text = model.seo_title;
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.seo_title = txtType.Text.Trim();
                ...
                if (bll.Add(model) >0)
                {
                    AddAdminLog(AXEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "频道栏目分类:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + this.channel_name + "_category", AXEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("channel_" + this.channel_name + "_category", AXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
