<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="dialog_print.aspx.cs" Inherits="Web.admin.dialog.dialog_print" %>
...
<tr>
  ...
  <asp:Repeater ID="rptList" runat="server">
      ...
      <FooterTemplate>
            <%#rptList.Items.Count == 0 ? "<tr><td align=\"center\" colspan=\"5\">" + Resources.lang.NoRecords+ "</td></tr>" : ""%>
          </table>
     </FooterTemplate>
  </asp:Repeater>
  </td>
...
<tr>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="5" style="margin:5px auto; font-size:12px; font-family:'微软雅黑'; background:#fff;">
    <tr>
      ...
      <td width="35%">联系人姓名：<%=model.accept_name%><br /></td>
         <td width="35%">支付方式：<%=new BLL.payment().GetTitle(model.payment_id) %></td>
    </tr>

      <tr>
           <td>商店号：<%=model.companyid%></td>
           <td>税号号：<%=model.taxid%></td>
            <td>邮编： <%=model.post_code%><br /></td>
     
    </tr>
        <tr>
      <td>微信号：<%=model.wechat%></td>
       <td valign="top">手&nbsp;&nbsp;&nbsp; 机：<%=model.mobile%></td>
         <td>电话：<%=model.telphone%></td>
    </tr>
      <tr>
           <td valign="top">城市：<%=model.city %></td>
      <td colspan="2">托运地址： <%=model.address%><br /></td>
      </tr>
          <tr>
      <td valign="top">公司名称：<%=model.companyname %></td>
      <td colspan="2">公司地址：<%=model.companyaddress%><br /></td>
    </tr>
    ...
    <tr>
      <td colspan="3">订单备注：<%=model.remark%></td>
    ...
    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" style="border-top:1px solid #000; font-size:12px; font-family:'微软雅黑'; background:#fff;">
      <tr>
        <td align="right">商品金额：<%=model.real_amount%> PLN + 配送费：<%=model.express_fee%> PLN + 支付手续费：<%=model.payment_fee%> PLN = 订单总额：<%=model.order_amount%> PLN</td>
      </tr>
    </table></td>
  </tr>
</table>
