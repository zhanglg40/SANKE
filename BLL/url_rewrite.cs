...
namespace BLL
{
    public class url_rewrite
    {
        ...
        #region 增、删、改" + Resources.lang.operating+ "=================================================
        /// <summary>
        /// 增加节点
        /// </summary>
        ...
        {
            Hashtable ht = CacheHelper.Get<Hashtable>(AXKeys.CACHE_SITE_URLS);
            if (ht == null)
            {
                CacheHelper.Insert(AXKeys.CACHE_SITE_URLS, dal.GetList(), Utils.GetXmlMapPath(AXKeys.FILE_URL_XML_CONFING));
                ht = CacheHelper.Get<Hashtable>(AXKeys.CACHE_SITE_URLS);
            }
            return ht;
        }
        ...
        {
            List<Model.url_rewrite> ls = CacheHelper.Get<List<Model.url_rewrite>>(AXKeys.CACHE_SITE_URLS_LIST);
            if (ls == null)
            {
                CacheHelper.Insert(AXKeys.CACHE_SITE_URLS_LIST, dal.GetList(""), Utils.GetXmlMapPath(AXKeys.FILE_URL_XML_CONFING));
                ls = CacheHelper.Get<List<Model.url_rewrite>>(AXKeys.CACHE_SITE_URLS_LIST);
            }
            return ls;
        }
