...
namespace Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "cart_goods_update": //购物车修改商品
                    ...
                    break;
                case "cart_goods_updateNew": //购物车修改商品
                    cart_goods_updateNew(context);
                    break;
                case "cart_goods_delete": //购物车删除商品
                    cart_goods_delete(context);
                    break;
                case "cart_goods_deleteNew": //购物车删除商品
                    cart_goods_deleteNew(context);
                    break;
                ...
                case "order_saveNew": //保存订单
                    order_saveNew(context);
                    break;
                case "set_user_address"://设置默认地址
                ...
                case "view_cart_count": //输出当前购物车总数
                    ...
                    break;
                case "change_languane": //输出当前购物车总数
                    change_languane(context);
                    break;

            }
        }
        private void change_languane(HttpContext context)
        {
        ...
        private void comment_add(HttpContext context)
        {
            ...
            string code = AXRequest.GetFormString("txtCode");
            int article_id = AXRequest.GetQueryInt("article_id");
            string _content = AXRequest.GetFormString("txtContent");
            //校检验证码
        ...
        {
            int article_id = AXRequest.GetQueryInt("article_id");
            int page_index = AXRequest.GetQueryInt("page_index");
            int page_size = AXRequest.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
        ...
        {
            string username = AXRequest.GetString("param");
            //如果为Null，退出
        ...
        {
            string code = AXRequest.GetFormString("txtCode").Trim();
            string invitecode = AXRequest.GetFormString("txtInviteCode").Trim();
            string username = Utils.ToHtml(AXRequest.GetFormString("txtUserName").Trim());
          //  string password = AXRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(AXRequest.GetFormString("txtEmail").Trim());
           string mobile = Utils.ToHtml(AXRequest.GetFormString("txtMobile").Trim());
            string userip = AXRequest.GetIP();
            string qq = Utils.ToHtml(AXRequest.GetFormString("txtQQ").Trim());
            string WeChat = Utils.ToHtml(AXRequest.GetFormString("txtWeChat").Trim());
            string addr = Utils.ToHtml(AXRequest.GetFormString("txtAddr").Trim());
         //   int express_id = AXRequest.GetFormInt("ddlExpressId");
            ...
            if (username == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"错误：用户名不能为空！\"}");
                return;
            }
            //if ((userConfig.regemailditto == 0 && email == "") && WeChat == "" && WeChat == "")
            //{
            //    context.Response.Write("{\"status\":0, \"msg\":\"错误：电子邮箱不能为空！\"}");
            //    return;
            //}
            if ((userConfig.regemailditto == 0 && email == "") && WeChat == "" && qq == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"错误：电子邮箱、QQ、微信请至少填写一项！\"}");
                return;
            }
            ...
            {
                if (bll.ExistsEmail(email)&&!string.IsNullOrEmpty(email))
                {
            ...
            string password = Utils.GetCheckCode(8); //获得8位的随机密码
         ...
         //   model.mobile = mobile;
            ...
            model.taxid = Utils.ToHtml(AXRequest.GetFormString("txtTaxid").Trim());
            model.companyid = Utils.ToHtml(AXRequest.GetFormString("txtCompanyid").Trim());
         //   model.express_id = express_id;
            ...
            {
                new BLL.user_amount_log().Add(model.id, model.user_name, AXEnums.AmountTypeEnum.SysGive.ToString(), modelGroup.amount, "注册赠送金额", 1);
            }
            ...
            else if (userConfig.regmsgstatus == 2) //发送邮件
            {
                ...
                if (mailModel != null)
                {
                    ...
                    //发送邮件
                    //AXMail.sendMail(siteConfig.emailsmtp, siteConfig.emailusername, siteConfig.emailpassword, siteConfig.emailnickname,
                    //    siteConfig.emailfrom, model.email, mailTitle, mailContent);
                    string senderServerIp = "123.125.50.133";
                    MyEmail myemail = new MyEmail(senderServerIp, model.email, siteConfig.emailusername, mailTitle, mailContent, siteConfig.emailnickname, siteConfig.emailpassword, siteConfig.emailport.ToString(), false, false);
                  //  myemail.AddAttachments(attachPath);
                    myemail.Send();
                }
            }
            ...
            {
                context.Session[AXKeys.SESSION_USER_INFO] = model;
                context.Session.Timeout = 45;

                //防止Session提前过期
                Utils.WriteCookie(AXKeys.COOKIE_USER_NAME_REMEMBER, "AoXiang", model.user_name);
                Utils.WriteCookie(AXKeys.COOKIE_USER_PWD_REMEMBER, "AoXiang", model.password);

                //写入登录日志
                new BLL.user_login_log().Add(model.id, model.user_name, "会员登录");


                context.Response.Write("{\"status\":1, \"url\":\"" + new Web.UI.BasePage().linkurl("register") + "?action=succeed&username=" + Utils.UrlEncode(model.user_name) + "&password=" + Utils.UrlEncode(password) + "\", \"msg\":\"注册成功，欢迎成为本站会员！\"}");
            }
            return;
        }
        ...
        private string verify_email(Model.users userModel)
        {
            ...
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, AXEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = AXEnums.CodeEnum.RegVerify.ToString();
            ...
            try
            {
                AXMail.sendMail(siteConfig.emailsmtp, 
        ...
        private string verify_mobile(Model.users userModel)
        {
            ...
            Model.user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, AXEnums.CodeEnum.RegVerify.ToString(), "n");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                codeModel.user_id = userModel.id;
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + AXEnums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + AXEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            ...
            codeModel.type = AXEnums.CodeEnum.Register.ToString();
        ...
        private void user_register_smscode(HttpContext context)
        {
            ...
            //写入SESSION，保存验证码
            context.Session[AXKeys.SESSION_SMS_CODE] = strcode;
        ...
        {
            string username = AXRequest.GetFormString("username");
            //检查是否过快
        ...
        private void user_login(HttpContext context)
        {
            ...
            string lang = cookie != null ? cookie.Value : "pl-PL";
            cookie = new HttpCookie("lang");
            string username = AXRequest.GetFormString("txtUserName");
            string password = AXRequest.GetFormString("txtPassword");
            string remember = AXRequest.GetFormString("chkRemember");
            //检查用户名密码
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
             ...
             if (lang.Equals("pl-PL"))
             {
                 context.Response.Write("{\"status\": 0, \"msg\": \"Wskazówki: Proszę podać nazwę użytkownika lub hasło！\"}");
             }
             else
             {
            ...
            if (model == null)
            {
                if (lang.Equals("pl-PL"))
                {
                    context.Response.Write("{\"status\": 0, \"msg\": \"Źle: nazwa użytkownika lub hasło jest nieprawidłowe, spróbuj ponownie！\"}");
                }
                else
                {
            ...
            {
                Utils.WriteCookie(AXKeys.COOKIE_USER_NAME_REMEMBER, "AoXiang", model.user_name, 43200);
                Utils.WriteCookie(AXKeys.COOKIE_USER_PWD_REMEMBER, "AoXiang", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = AXRequest.GetString("txtUserName");
            string password = AXRequest.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
        ...
        #endregion
        #region 设置默认地址OK=================================
        private void set_user_address(HttpContext context)
        {
            //检查用户是否登录
            Model.users model = new BasePage().GetUserInfo();
            if (model == null)
            ...
            string id = AXRequest.GetQueryString("id");
            Model.user_address modelad = new BLL.user_address().GetModel(int.Parse(id));
            model.address = modelad.address;
            model.telphone = modelad.telphone;
            model.mobile = modelad.mobile;
            model.nick_name = modelad.nick_name;
            new BLL.users().Update(model);
            context.Response.Write("{\"status\":1, \"msg\":\"设置默认地址成功！\"}");
            return;
        }
        #endregion
        #region 修改用户信息OK=================================
        private void user_info_edit(HttpContext context)
        {
            ...
            string email = Utils.ToHtml(AXRequest.GetFormString("txtEmail"));
            string nick_name = Utils.ToHtml(AXRequest.GetFormString("txtNickName"));
            string sex = AXRequest.GetFormString("rblSex");
            string birthday = AXRequest.GetFormString("txtBirthday");
            string telphone = Utils.ToHtml(AXRequest.GetFormString("txtTelphone"));
            string mobile = Utils.ToHtml(AXRequest.GetFormString("txtMobile"));
            string qq = Utils.ToHtml(AXRequest.GetFormString("txtQQ"));
            ...
            string companyname = Utils.ToHtml(context.Request.Form["txtComName"]);
            string companyaddress = Utils.ToHtml(context.Request.Form["txtComAddress"]);
            string companyid = Utils.ToHtml(context.Request.Form["txtComID"]);
            string wechat = Utils.ToHtml(context.Request.Form["txtWeChat"]);
            string taxid = Utils.ToHtml(context.Request.Form["txtTaxid"]);
            //检查姓名
            if (nick_name == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入您的姓名姓名！\"}");
                return;
            }
            //检查邮箱
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = AXRequest.GetFormString("hideFileName");
            int x1 = AXRequest.GetFormInt("hideX1");
            int y1 = AXRequest.GetFormInt("hideY1");
            int w = AXRequest.GetFormInt("hideWidth");
            int h = AXRequest.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = AXRequest.GetFormString("txtOldPassword");
            string password = AXRequest.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        {
            string code = AXRequest.GetFormString("txtCode");
            string username = AXRequest.GetFormString("txtUserName").Trim();
            //检查用户名是否正确
            if (string.IsNullOrEmpty(username))
            {
            ...
            BLL.user_code codeBll = new BLL.user_code();
            Model.user_code codeModel;
            codeModel = codeBll.GetModel(username, AXEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Model.user_code();
                //写入数据库
                codeModel.user_id = model.id;
                codeModel.user_name = model.user_name;
                codeModel.type = AXEnums.CodeEnum.Password.ToString();
                codeModel.str_code = strcode;
                codeModel.eff_time = DateTime.Now.AddDays(userConfig.regemailexpired);
                codeModel.add_time = DateTime.Now;
                codeBll.Add(codeModel);
        ...
        private void user_message_delete(HttpContext context)
        {
            ...
            string check_id = AXRequest.GetFormString("checkId");
            if (check_id == "")
            {
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string code = context.Request.Form["txtCode"];
            string send_save = AXRequest.GetFormString("sendSave");
            string user_name = Utils.ToHtml(AXRequest.GetFormString("txtUserName"));
            string title = Utils.ToHtml(AXRequest.GetFormString("txtTitle"));
            string content = Utils.ToHtml(AXRequest.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = AXRequest.GetFormInt("txtAmount");
            string password = AXRequest.GetFormString("txtPassword");
            if (model.amount < 1)
            {
            ...
            //扣除金额
            int amountNewId = new BLL.user_amount_log().Add(model.id, model.user_name, AXEnums.AmountTypeEnum.Convert.ToString(), amout * -1, "用户兑换积分", 1);
            //增加积分
            if (amountNewId < 1)
            {
            ...
            {
                //返还金额
                new BLL.user_amount_log().Add(model.id, model.user_name, AXEnums.AmountTypeEnum.Convert.ToString(), amout, "用户兑换积分失败，返还金额", 1);
                context.Response.Write("{\"status\":0, \"msg\":\"转换过程中发生错误，请重新提交！\"}");
                return;
            }
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            decimal amount = AXRequest.GetFormDecimal("order_amount", 0);
            int payment_id = AXRequest.GetFormInt("payment_id");
            if (amount == 0)
            {
            ...
            new BLL.user_amount_log().Add(model.id, model.user_name, AXEnums.AmountTypeEnum.Recharge.ToString(), order_no,
        ...
        private void user_address_delete(HttpContext context)
        {
            ...
            for (int i = 0; i < arrId.Length; i++)
            {
                ...
                new BLL.user_address().Delete(int.Parse(arrId[i]));
            }
            context.Response.Write("{\"status\":1, \"msg\":\"地址删除成功！\"}");
            return;
        }
         #endregion
        #region 购物车加入商品OK===============================
        ...
        private void cart_goods_update(HttpContext context)
        {
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"商品数量更改失败，请检查" + Resources.lang.operating+ "是否有误！\"}");
            }
            return;
        }
        #endregion
        #region 修改购物车商品OK===============================
        private void cart_goods_updateNew(HttpContext context)
        {
            string shop_id = AXRequest.GetFormString("shop_id");
            string id = AXRequest.GetFormString("id");
            int goods_quantity = AXRequest.GetFormInt("goods_quantity");
            ...
            if (Web.UI.ShopCart.UpdateNew(Convert.ToInt16(id), goods_quantity))
            {
                context.Response.Write("{\"status\":1, \"msg\":\"商品数量修改成功！\"}");
            }
            else
        ...
        #endregion
        #region 删除购物车商品OK===============================
        private void cart_goods_deleteNew(HttpContext context)
        {
            string id = AXRequest.GetFormString("id");
            if (id == "")
            {
            ...
            Web.UI.ShopCart.Delete(Convert.ToInt16(id));
         //   Web.UI.ShopCart.Clear(goods_id);
            context.Response.Write("{\"status\":1, \"msg\":\"商品移除成功！\"}");
            return;
        }
        #endregion
        private void order_saveNew(HttpContext context)
        {
            //false匿名
            bool noname = true;
            //检查用户是否登录
            Model.users userModel = new BasePage().GetUserInfo();
            if (userModel == null) noname = false;
            //获取订单配置信息
            ...
            DataSet ds = shop.GetList("a.user_id=" + userModel.id, "shop_id");
            DataTable dt=ds.Tables[0];
            if (dt.Rows.Count == 0)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，购物车为空！\"}");
                return;
            }
            DataView dataView = dt.DefaultView;
            string orders = "";
            // 得到商店个数
            DataTable dataTableDistinct = dataView.ToTable(true, "companyid");
            for (int i = 0; i < dataTableDistinct.Rows.Count; i++)
            {


                DataRow[] drArr = dt.Select("companyid='" + dataTableDistinct.Rows[i]["companyid"] + "'");
                Model.orders model = new Model.orders();
                model.order_no = "B" + Utils.GetOrderNumber(); //订单号B开头为商品订单
                orders = orders + model.order_no + "  ";
                model.user_id = userModel.id;
                model.user_name = userModel.nick_name;
                model.taxid = userModel.taxid;
                model.ref1 = userModel.ref1;
                model.payment_id = 1;
                model.express_id = 1;
                model.accept_name = drArr[0]["accept_name"].ToString();
                model.wechat = userModel.wechat;
                model.companyaddress = drArr[0]["companyaddress"].ToString();
                model.companyid = drArr[0]["companyid"].ToString();
                model.companyname = drArr[0]["companyname"].ToString();
                model.telphone = drArr[0]["telphone"].ToString();
                model.mobile = drArr[0]["telphone"].ToString();
                model.address = drArr[0]["companyaddress"].ToString();
                model.message = Utils.ToHtml(AXRequest.GetFormString("message"));
               // model.payable_amount =Convert.ToDecimal( dt.Compute("Sum(quantity*sell_price)", "companyid='" + dataTableDistinct.Rows[i]["companyid"] + "'"));
               // model.real_amount = model.payable_amount;
                model.post_code = "";
                model.city = drArr[0]["city"].ToString(); ;
                ...
                {
                    dd = dd + Convert.ToDecimal(dr["sell_price"]) * Convert.ToInt16(dr["quantity"]);
                    gls.Add(new Model.order_goods { goods_id = Convert.ToInt16(dr["goods_id"]), goods_title = dr["title"].ToString(), goods_price =Convert.ToDecimal( dr["sell_price"]), real_price =Convert.ToDecimal( dr["sell_price"]), quantity = Convert.ToInt16(dr["quantity"]), point = 0, Barcode = dr["Barcode"].ToString(), english_name = dr["english_name"].ToString() });

                }
                model.payable_amount = dd;
                model.real_amount = model.payable_amount;
                //订单总金额=实付商品金额+运费+支付手续费
                model.order_amount = model.real_amount + model.express_fee + model.payment_fee;
                ...
                //削减库存
                foreach (Model.order_goods g in gls)
                {
                    art.updateQuantity(g.goods_id, g.quantity);
                }
                //发送邮件
                if (siteConfig.neworderstatus == 1)
                {
                    //取得邮件模板内容
                    Model.mail_template mailModel = new BLL.mail_template().GetModel("order_new");
                    if (mailModel == null)
                    {
                    ...
                    mailContent = mailContent.Replace("{time}", model.add_time.ToString("yyyy-MM-dd HH:mm"));
                    mailContent = mailContent.Replace("{webname}", "波兰贸易国");
                    string password = DESEncrypt.Decrypt(siteConfig.emailpassword, siteConfig.sysencryptstring);
                    try
                    {
                        //if (siteConfig.mailtype == 2)
                        //{
                        //发送邮件
                        AXMail.sendMail2(siteConfig.emailsmtp, siteConfig.emailusername, password, siteConfig.emailnickname,
                            siteConfig.emailfrom, siteConfig.neworderemail, mailTitle, mailContent);
                        //}
                        //else
                        //{
                        //    //发送邮件
                        //    AXMail.sendMail(siteConfig.emailsmtp, siteConfig.emailusername, password, siteConfig.emailnickname,
                        //        siteConfig.emailfrom, siteConfig.neworderemail, mailTitle, mailContent);
                        //}
            ...
            //清空购物车
            shop.DeleteAll(userModel.id);
            //提交成功，返回URL
       //     context.Response.Write("{\"status\":1, \"url\":\"" + new Web.UI.BasePage().linkurl("payment", "confirm", orders) + "\", \"msg\":\"恭喜您，订单已成功提交！\"}");
            context.Response.Write("{\"status\":1, \"url\":\"/payment.aspx?action=succeed&order_no=" + orders + "\", \"msg\":\"恭喜您，订单已成功提交！\"}");
            return;
        ...
        private void order_save(HttpContext context)
        {
            ...
            string accept_name = Utils.ToHtml(AXRequest.GetFormString("accept_name"));
            string wechat = Utils.ToHtml(AXRequest.GetFormString("wechat"));
            string telphone = Utils.ToHtml(AXRequest.GetFormString("telphone"));
            string mobile = Utils.ToHtml(AXRequest.GetFormString("mobile"));
            string address = Utils.ToHtml(AXRequest.GetFormString("address"));
            string message = Utils.ToHtml(AXRequest.GetFormString("message"));
            string companyid = Utils.ToHtml(AXRequest.GetFormString("companyid"));
            string companyname = Utils.ToHtml(AXRequest.GetFormString("companyname"));
            string companyaddress = Utils.ToHtml(AXRequest.GetFormString("companyaddress"));
            string city = Utils.ToHtml(AXRequest.GetFormString("city"));
            string post_code = Utils.ToHtml(AXRequest.GetFormString("post_code"));
            //获取订单配置信息
            Model.orderconfig orderConfig = new BLL.orderconfig().loadConfig();
            ...
            //if (express_id == 0)
            //{
            //    context.Response.Write("{\"status\":0, \"msg\":\"对不起，请选择配送小区！\"}");
            //    return;
            //}
            Model.express expModel = new BLL.express().GetModel(express_id);
            //if (expModel == null)
            //{
            //    context.Response.Write("{\"status\":0, \"msg\":\"对不起，配送小区不存在或已删除！\"}");
            //    return;
            //}
            //检查支付方式
            if (payment_id == 0)
            ...
            //检查" + Resources.lang.Receiver+ "
            if (string.IsNullOrEmpty(accept_name))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入" + Resources.lang.Receiver+ "姓名！\"}");
                return;
            }
            //检查手机和电话
            if (string.IsNullOrEmpty(telphone) && string.IsNullOrEmpty(mobile))
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入" + Resources.lang.Receiver+ "联系电话或手机！\"}");
                return;
            }
            //检查公司地址
            if (string.IsNullOrEmpty(companyaddress) && noname)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入公司地址！\"}");
                return;
            }
            //检查公司名称
            if (string.IsNullOrEmpty(companyname) && noname)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入公司名称！\"}");
                return;
            }
            //检查商店号
            if (string.IsNullOrEmpty(companyid) && noname)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，请输入商店号！\"}");
                return;
            }
            //检查地址
            if (string.IsNullOrEmpty(address) && noname)
            {
            ...
            string user_taxid = string.Empty;
            string user_ref1 = string.Empty;
            ...
            if (userModel != null)
            {
                ...
                user_name = userModel.user_name;
                user_taxid = userModel.taxid;
                user_ref1 = userModel.ref1;
            }
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            Model.orders model = new Model.orders();
            model.order_no = "B" + Utils.GetOrderNumber(); //订单号B开头为商品订单
            model.user_id = user_id;
            model.user_name = user_name;
            model.taxid = user_taxid;
            model.ref1 = user_ref1;
           ...
           // model.express_fee = expModel.express_fee; //物流费用
            ...
            {
                gls.Add(new Model.order_goods { goods_id = item.id, goods_title = item.title, goods_price = item.price, real_price = item.user_price, quantity = item.quantity, point = item.point, Barcode = item.Barcode, english_name=item.english_name });
    ...
    /*        System.Net.WebClient wCient = new System.Net.WebClient();
       ...
       byte[] postData = System.Text.Encoding.ASCII.GetBytes("id=" + "1");

       byte[] responseData = wCient.UploadData("http://123.56.146.7/api/process/getlist?content=dasd", "POST", postData);
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = AXRequest.GetQueryString("order_no");
            ...
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，不能取消别人的" + Resources.lang.orderStatus+ "！\"}");
                return;
            }
            //检查" + Resources.lang.orderStatus+ "
            ...
            if (!result)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，" + Resources.lang.operating+ "过程中发生不可遇知的错误！\"}");
                return;
            }
            foreach(Model.order_goods good in orderModel.order_goods){
                new BLL.article().updateAttribute(good.goods_id, "stock_quantity=stock_quantity+" + good.quantity);
            }
        ...
        {
            int article_id = AXRequest.GetInt("id", 0);
            int click = AXRequest.GetInt("click", 0);
            int count = 0;
            if (article_id > 0)
            {
        ...
        {
            int attach_id = AXRequest.GetInt("id", 0);
            int count = 0;
            if (attach_id > 0)
            {
        ...
        private string verify_code(HttpContext context,string strcode)
        {
            ...
            if (context.Session[AXKeys.SESSION_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[AXKeys.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[AXKeys.SESSION_CODE] = null;
            return "success";
        }
        //校检网站验证码
        private string verify_sms_code(HttpContext context, string strcode)
        {
            ...
            if (context.Session[AXKeys.SESSION_SMS_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[AXKeys.SESSION_SMS_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[AXKeys.SESSION_SMS_CODE] = null;
            return "success";
        }
        #endregion END通用方法=================================================
