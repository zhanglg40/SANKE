...
namespace Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        private void ModeBind()
        {
            ...
            this.field_control_PriceMode.Items.Clear();
            this.field_control_PriceMode.Items.Add(new ListItem("请选择价格模式..", ""));
            this.field_control_PriceMode.Items.Add(new ListItem("0:批发价", "0"));
            this.field_control_PriceMode.Items.Add(new ListItem("1:折扣价", "1"));
            this.field_control_PriceMode.Items.Add(new ListItem("2:促销价", "2"));


            div_PriceMode.Visible = true;
        }
        #endregion
            ...
            foreach (Model.article_attribute_field modelt1 in ls1)
            {
                switch (modelt1.control_type)
                {
                    ...
                    case "multi-text": //多行文本
                        goto case "single-text";
                    case "combobox"://选择框
                        DropDownList comboControl = FindControl("field_control_" + modelt1.name) as DropDownList;
                        if (comboControl != null && model.fields.ContainsKey(modelt1.name))
                        {
                            comboControl.SelectedValue = model.fields[modelt1.name];
                        }
                        break;
                    case "editor": //编辑器
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                switch (dr["control_type"].ToString())
                {
                    case "single-text": //单行文本
                        ...
                        break;
                    case "combobox":
                        DropDownList ddControl = FindControl("field_control_" + dr["name"].ToString()) as DropDownList;
                        if(ddControl!=null)
                            dic.Add(dr["name"].ToString(), ddControl.SelectedValue.Trim());
                        break;
                    case "multi-text": //多行文本
                        goto case "single-text";
                    case "editor": //编辑器
        ...
        #region 增加操作=================================
        private bool DoAdd( ref string message)
        {
            bool result = false;
            ...
            model.vote_id = Utils.StrToInt(ddlvote_id.SelectedValue, 0);

            model.is_sys = 1; //管理员发布
            model.user_name = "admin"; //获得当前登录用户名
            model.add_time = Utils.StrToDateTime(txtAddTime.Text.Trim());
            model.fields = SetFieldValues(this.channel_id); //扩展字段赋值
            if (model.fields.ContainsKey("PriceMode"))
            {
                if (model.fields["PriceMode"] == "" || model.fields["PriceMode"] == null)
                {
                    message = "价格模式不能为空";
                    return false;
                }
                ...
                if (model.fields["PurchasePrice"] == "" || model.fields["PurchasePrice"] == null)
                {
                    message = "进货价不能为空";
                    return false;
                }
                if (model.fields["WholesalePrice"] == "" || model.fields["WholesalePrice"] == null)
                {
                    message = "批发价不能为空";
                    return false;
                }
                if (model.fields["SpecialPrice"] == "" || model.fields["SpecialPrice"] == null)
                {
                    message = "特别价不能为空";
                    return false;
                }
                if (model.fields["RetailPrice"] == "" || model.fields["RetailPrice"] == null)
                {
                    message = "零售价不能为空";
                    return false;
                }
                if (model.fields["TaxRate"] == "" || model.fields["TaxRate"] == null)
                {
                    message = "税率不能为空";
                    return false;
                }
                if (model.fields["DiscountRate"] == "" || model.fields["DiscountRate"] == null)
                {
                    message = "折扣率不能为空";
                    return false;
                }
            ...
            model.fields["sub_title"] = txtTitle.Text.Trim();
            #region 保存相册====================
            //检查是否有自定义图片
            ...
            for (int i = 0; i < rptPrice.Items.Count; i++)
            {
                ...
                decimal _price = Convert.ToDecimal(((TextBox)rptPrice.Items[i].FindControl("txtGroupPrice")).Text.Trim());
                 _price = Convert.ToDecimal(model.fields["sell_price"]);
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(AXEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        #region 修改操作=================================
        private bool DoEdit(int _id, ref string message)
        {
            bool result = false;
            BLL.article bll = new BLL.article();
            Model.article model = bll.GetModel(_id);
         ...
         model.fields["sub_title"] = txtTitle.Text.Trim();
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(AXEnums.ActionEnum.Edit.ToString(), "修改" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (action == AXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", AXEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id, ref message))
                {
                    if (message == "")
                    {
            ...
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", AXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd(ref message))
                {
                    if (message == "")
                    {
                        JscriptMsg("保存过程中发生错误啦！", "", "Error");
