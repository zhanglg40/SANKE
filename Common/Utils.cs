...
namespace Common
{
    public class Utils
    {
        ...
        {
            return AXKeys.ASSEMBLY_VERSION;
        }
        #endregion
        ...
        #region 文件" + Resources.lang.operating+ "
        /// <summary>
        /// 删除单个文件
        /// </summary>
        ...
        public static string OutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            ...
            {
                return OutPageListPL(pageSize, pageIndex, totalCount, linkUrl, centSize);
            }
            //计算页数
        ...
        #endregion
        public static string OutPageListPL(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            StringBuilder pageStr = new StringBuilder();
            string pageId = "__id__";
            string firstBtn = "<a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex - 1).ToString()) + "\">«poprzedni</a>";
            string lastBtn = "<a href=\"" + ReplaceStr(linkUrl, pageId, (pageIndex + 1).ToString()) + "\">następny»</a>";
            string firstStr = "<a href=\"" + ReplaceStr(linkUrl, pageId, "1") + "\">1</a>";
            string lastStr = "<a href=\"" + ReplaceStr(linkUrl, pageId, pageCount.ToString()) + "\">" + pageCount.ToString() + "</a>";

            if (pageIndex <= 1)
            {
                firstBtn = "<span class=\"disabled\">«poprzednia</span>";
            }
            if (pageIndex >= pageCount)
            {
                lastBtn = "<span class=\"disabled\">następna»</span>";
            }
            if (pageIndex == 1)
            {
                firstStr = "<span class=\"current\">1</span>";
            ...
            if (lastNum >= pageCount)
                lastNum = pageCount - 1;
            pageStr.Append("<span>Towar ogółem " + totalCount + "</span>");
            pageStr.Append(firstBtn + firstStr);
            if (pageIndex >= centSize)
            {
                pageStr.Append("<span>...</span>\n");
        ...
        #region " + Resources.lang.operating+ "权限菜单
        /// <summary>
        /// 获取" + Resources.lang.operating+ "权限
        /// </summary>
