...
namespace DAL
{
    ...
    public partial class article_category
    {
        ...
        public string GetSeoTitle(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 seo_title from " + databaseprefix + "article_category");
            strSql.Append(" where id=" + id);
            string title = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(title))
            {
        ...
        /// 通过名称(classtype)返回ID 
        /// </summary>
        public int GetID(string classtype)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "article_category");
            strSql.Append(" where class_type='" + classtype.Trim()+"'");
            string id = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(id))
            {
                return 0;
        ...
        /// 通过编号返回ID
        /// </summary>
        public int GetIDbyNo(string classno)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "article_category");
            strSql.Append(" where class_no='" + classno.Trim() + "'");
            string id = Convert.ToString(DbHelperSQL.GetSingle(strSql.ToString()));
            if (string.IsNullOrEmpty(id))
            {
                return 0;
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,class_no,class_type,remark,seo_description)");
                        ...
                        SqlParameter[] parameters = {
					            ...
					            new SqlParameter("@class_type", SqlDbType.NVarChar,255),
					            new SqlParameter("@remark", SqlDbType.NVarChar,5),
                        ...
                        parameters[12].Value = model.class_no;
                        parameters[13].Value = model.class_type;
        ...
        public Model.article_category GetModel(int id)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["class_type"] != null && ds.Tables[0].Rows[0]["class_type"].ToString() != "")
                {
                    model.class_type = ds.Tables[0].Rows[0]["class_type"].ToString();
                }
                if (ds.Tables[0].Rows[0]["class_no"] != null && ds.Tables[0].Rows[0]["class_no"].ToString() != "")
                {
                    model.class_no = ds.Tables[0].Rows[0]["class_no"].ToString();
                }
                return model;
            }
            else
        ...
        /// <param name="channel_id">频道ID</param>
        /// <returns></returns>
        public DataTable GetChildListpp(int Top, int parent_id, int channel_id, int category_id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description from " + databaseprefix + "article_category");
            if (Top == 200)
            {
                strSql.Append(" where channel_id=" + channel_id + " and parent_id=" + parent_id + "");
            }
            else
            {
                strSql.Append(" where channel_id=" + channel_id + " and parent_id=" + parent_id + " and img_url<>'' ");
            }
            if (category_id > 0)
            {
                strSql.Append(" and id in(select vote_id from " + databaseprefix + "article where category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%'))");         
            }
            else
            {
                strSql.Append(" and id in(select vote_id from " + databaseprefix + "article where vote_id<>0 )");
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,channel_id,title,call_index,parent_id,class_list,class_layer,class_no,remark,class_type,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description from " + databaseprefix + "article_category");
        ...
        private void GetChilds(DataTable oldData, DataTable newData, int parent_id, int channel_id)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                if (dr[i]["class_no"].ToString()!=""&&dr[i]["class_no"].ToString()!=null)
                row["class_no"] = int.Parse(dr[i]["class_no"].ToString());
