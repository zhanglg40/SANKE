...
namespace DAL
{
    ...
    public partial class article
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "article_attribute_value set " + strValue);
                    ...
                    try
                    {
                        ...
                        if (model.fields.Count > 0)
                        {
                            ...
                            foreach (KeyValuePair<string, string> kvp in model.fields)
                            {
                                ...
                                {
                                    parameters2[i].Value = null;
                                }
                              else  parameters2[i].Value = kvp.Value;
                                i++;
                            }
        ...
        public Model.article GetModelNew(string sqlwhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,category_id,call_index,title,link_url,img_url,seo_title,seo_keywords,seo_description,zhaiyao,content,sort_id,click,status,groupids_view,vote_id,is_top,is_red,is_hot,is_slide,is_sys,is_msg,user_name,add_time,update_time");
            strSql.Append(" from view_channel_goods ");
        ...
        public void updateQuantity(int id,int strValue)
        {
              StringBuilder strSql = new StringBuilder();
              strSql.Append("update " + databaseprefix + "article_attribute_value set stock_quantity=stock_quantity-" + strValue);
              strSql.Append(" where article_id=" + id);
            DbHelperSQL.ExecuteSql(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "article a," + databaseprefix + "article_attribute_value b");
            if (channel_id > 0)
            {
                strSql.Append(" where a.id=b.article_id and channel_id=" + channel_id);
            }
            if (category_id > 0)
            {
                if (channel_id > 0)
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  view_channel_goods");
        ...
        public DataSet GetListgood(int pageSize, int pageIndex, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM view_channel_goods" );
            recordCount = Convert.ToInt32(DbHelperSQL.GetSingle(PagingHelper.CreateCountingSql(strSql.ToString())));
            string sql = " SELECT * FROM  (  SELECT TOP " + pageSize
                + " * FROM  (  SELECT TOP " + pageIndex * pageSize + " *  FROM (select * FROM view_channel_goods) AS T  ORDER BY sort_id,update_date desc ) AS T  ORDER BY sort_id DESC,update_date asc  ) AS T  ORDER BY sort_id ,update_date desc";
            return DbHelperSQL.Query(sql);
        }
        public DataSet GetListthreshold(int pageSize, int pageIndex, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM view_channel_threshold");
            recordCount = Convert.ToInt32(DbHelperSQL.GetSingle(PagingHelper.CreateCountingSql(strSql.ToString())));
            return DbHelperSQL.Query(PagingHelper.CreatePagingSql(recordCount, pageSize, pageIndex, strSql.ToString(), "sort_id"));
        }
        /// <summary>
        /// 根据视图获得查询分页数据
        /// </summary>
        public DataSet GetListpp(string channel_name, int category_id, int vote_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM view_channel_" + channel_name);
            strSql.Append(" where datediff(d,add_time,getdate())>=0");
            if (vote_id > 0)
            {
                strSql.Append(" and vote_id = " + vote_id + "");
            }
            if (category_id > 0)
            {
                strSql.Append(" and category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%')");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM  view_channel_goods a");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
