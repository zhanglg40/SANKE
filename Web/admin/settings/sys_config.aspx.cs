...
namespace Web.admin.settings
{
    public partial class sys_config : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_config", AXEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        private void ShowInfo()
        {
            ...
            if (model.neworderstatus == 1)
            {
                neworderstatus.Checked = true;
            }
            else
            {
                neworderstatus.Checked = false;
            }
            neworderemail.Text = model.neworderemail;
        ...
        {
            ChkAdminLevel("site_config", AXEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.siteconfig bll = new BLL.siteconfig();
            Model.siteconfig model = bll.loadConfig();
            try
            {
                ...
                model.neworderemail = neworderemail.Text;
                if (neworderstatus.Checked == true)
                {
                    model.neworderstatus = 1;
                }
                else
                {
                    model.neworderstatus = 0;
                }
                ...
                AddAdminLog(AXEnums.ActionEnum.Edit.ToString(), "修改系统配置信息"); //记录日志
