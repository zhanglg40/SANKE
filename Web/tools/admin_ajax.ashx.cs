...
namespace Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "navigation_validate": //验证导航菜单ID是否重复
                    ...
                    break;
                case "manager_validate": //验证管理员" + Resources.lang.username+ "是否重复
                ...
                case "edit_order_status": //修改订单信息和状态
                    ...
                    break;
                case "validate_username": //验证会员" + Resources.lang.username+ "是否重复
        ...
        {
            string column_name = AXRequest.GetString("param");
        ...
        {
            string build_path = AXRequest.GetString("param");
            string old_build_path = AXRequest.GetString("old_build_path");
        ...
        {
            string channel_name = AXRequest.GetString("param");
            string old_channel_name = AXRequest.GetString("old_channel_name");
        ...
        {
            string new_name = AXRequest.GetString("param");
            string old_name = AXRequest.GetString("old_name");
        ...
        {
            string navname = AXRequest.GetString("param");
            string old_name = AXRequest.GetString("old_name");
        ...
        #region 验证管理员" + Resources.lang.username+ "是否重复========================
        private void manager_validate(HttpContext context)
        {
            string user_name = AXRequest.GetString("param");
            if (string.IsNullOrEmpty(user_name))
            {
                context.Response.Write("{ \"info\":\"请输入" + Resources.lang.username+ "\", \"status\":\"n\" }");
                return;
            }
            BLL.manager bll = new BLL.manager();
            if (bll.Exists(user_name))
            {
                context.Response.Write("{ \"info\":\"" + Resources.lang.username+ "已被占用，请更换！\", \"status\":\"n\" }");
                return;
            }
            context.Response.Write("{ \"info\":\"" + Resources.lang.username+ "可使用\", \"status\":\"y\" }");
            return;
        }
        #endregion
        ...
        {
            string filePath = AXRequest.GetFormString("remotepath");
        ...
        private void get_navigation_list(HttpContext context)
        {
            ...
            DataTable dt = new BLL.navigation().GetDataList(0, AXEnums.NavigationEnum.System.ToString());
        ...
        private void edit_order_status(HttpContext context)
        {
            ...
            switch (edit_type.ToLower())
            {
                case "order_confirm": //确认订单
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Confirm.ToString()))
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Confirm.ToString(), "确认订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                        ...
                        switch (orderConfig.confirmmsg)
                        {
                            ...
                            case 2: //邮件通知
                                ...
                                if (model.user_id > 0)
                                {
                                    ...
                                    //MyEmail myemail = new MyEmail(siteConfig.emailsmtp, userModel.email, siteConfig.emailfrom, mailTitle, mailContent + "qqqqqqqqqqqqqqqqqqqqqqq", siteConfig.emailnickname, siteConfig.emailpassword, siteConfig.emailport.ToString(), false, false);
                ...
                case "order_payment": //确认付款
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Confirm.ToString(), "确认付款订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                    ...
                    break;
                case "order_express": //确认送货
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Confirm.ToString()))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"您没有确认送货的权限！\"}");
                        return;
                    }
                    if (model.status > 2 || model.express_status == 2)
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"订单已完成或已送货，不能重复处理！\"}");
                        return;
                    }
                    ...
                    //if (express_id == 0)
                    //{
                    //    context.Response.Write("{\"status\": 0, \"msg\": \"请选择配送小区！\"}");
                    //    return;
                    //}
                    ...
                    if (!bll.Update(model))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"订单送货失败！\"}");
                        return;
                    }
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Confirm.ToString(), "确认送货订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    ...
                    #endregion
                    context.Response.Write("{\"status\": 1, \"msg\": \"订单送货成功！\"}");
                    break;
                case "order_complete": //完成订单=========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Confirm.ToString(), "确认交易完成订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                ...
                case "order_cancel": //取消订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Cancel.ToString()))
                    {
                    ...
                    int check_revert1 = AXRequest.GetFormInt("check_revert");
                    if (check_revert1 == 1)
                    {
                        //如果存在积分换购则返还会员积分
                        if (model.user_id > 0 && model.point < 0)
                        ...
                        {
                            new BLL.user_amount_log().Add(model.user_id, model.user_name, AXEnums.AmountTypeEnum.BuyGoods.ToString(), model.order_amount, "取消订单退还金额，订单号：" + model.order_no);
                        }

                    }
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Cancel.ToString(), "取消订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"取消订单成功！\"}");
                    break;
                case "order_invalid": //作废订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Invalid.ToString()))
                    {
                    ...
                    int check_revert2 = AXRequest.GetFormInt("check_revert");
                    if (check_revert2 == 1)
                    {
                        //扣除购物赠送的积分
                        if (model.user_id > 0 && model.point > 0)
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Invalid.ToString(), "作废订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"作废订单成功！\"}");
                    break;
                case "edit_accept_info": //修改收货信息====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Edit.ToString()))
                    {
                    ...
                    string post_code = AXRequest.GetFormString("post_code");
                    string mobile = AXRequest.GetFormString("mobile");
                    string telphone = AXRequest.GetFormString("telphone");

                    if (accept_name == "")
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"请填写" + Resources.lang.Receiver+ "姓名！\"}");
                        return;
                    }
                    if (area == "")
                    {
                    ...
                    if (!bll.Update(model))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"修改" + Resources.lang.Receiver+ "信息失败！\"}");
                        return;
                    }
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "修改收货信息，订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"修改" + Resources.lang.Receiver+ "信息成功！\"}");
                    break;
                case "order_delete": //刪除商品====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Edit.ToString()))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"您没有修改商品內容的权限！\"}");
                        return;
                    }
                    if (model.status == 2)
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"订单已经確認，不能刪除商品！\"}");
                        return;
                    }
                    string id = AXRequest.GetFormString("id");
                    //修改訂單總金額
                    model.real_amount = model.real_amount - new BLL.orders().GetGoodsRealAmount(id);
                    if (!bll.Update(model))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"刪除商品信息失敗！\"}");
                        return;
                    }
                    ////刪除單據
                    new BLL.orders().DeleteGoods(id, adminInfo.id, adminInfo.user_name);

                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "刪除商品，订单号:" + model.order_no + ",ID:" + id); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"刪除商品成功！\"}");
                    break;
                case "order_edit": //修改商品====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Edit.ToString()))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"您没有修改商品內容的权限！\"}");
                    ...
                    if (model.status == 2)
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"订单已经確認，不能修改商品數量！\"}");
                        return;
                    }
                    id = AXRequest.GetFormString("id");
                    int num =Convert.ToInt32( AXRequest.GetFormString("num"));
                    //判定庫存是否足夠
                    if (!new BLL.orders().ExistsGoodsNum(id, num,"edit"))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"商品庫存不夠！\"}");
                        return;
                    }
                    //修改訂單總金額
                    model.real_amount = model.real_amount + new BLL.orders().GetGoodsChangeAmount(id, num);
                    if (!bll.Update(model))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"修改商品數量失敗！\"}");
                        return;
                    }
                    //更新商品數量
                    new BLL.orders().UpdateGoodsNum(id, num, adminInfo.id, adminInfo.user_name);

                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "更改商品數量，订单号:" + model.order_no + ",ID:" + id + ",修改後數量:" + num); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"修改商品數量成功！\"}");
                    break;
                case "order_add": //新添商品====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AXEnums.ActionEnum.Edit.ToString()))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"您没有新添商品的权限！\"}");
                        return;
                    }
                    if (model.status == 2)
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"订单已经確認，不能新添商品！\"}");
                        return;
                    }
                    id = AXRequest.GetFormString("id");
                    num = AXRequest.GetFormInt("num");
                    decimal amount = AXRequest.GetFormDecimal("amount",0);
                    //判定庫存是否足夠
                    if (!new BLL.orders().ExistsGoodsNum(id, num,"add"))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"商品庫存不夠！\"}");
                        return;
                    }
                    //修改訂單總金額
                    model.real_amount = model.real_amount + amount;
                    if (!bll.Update(model))
                    {
                        context.Response.Write("{\"status\": 0, \"msg\": \"添加商品失敗！\"}");
                        return;
                    }
                    //添加商品
                    new BLL.orders().AddGoods(model.id,id, num);

                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "添加商品數量，订单号:" + model.order_no + ",商品ID:" + id + ",添加數量:" + num+",金額"+amount); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"添加商品成功！\"}");
                    break;
                case "edit_order_remark": //修改订单备注=================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "修改订单备注，订单号:" + model.order_no); //记录日志
                ...
                case "edit_real_amount": //修改商品总金额================================
                    ...
                    decimal real_amount = AXRequest.GetFormDecimal("real_amount", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "修改商品金额，订单号:" + model.order_no); //记录日志
                ...
                case "edit_express_fee": //修改配送费用==================================
                    ...
                    decimal express_fee = AXRequest.GetFormDecimal("express_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "修改配送费用，订单号:" + model.order_no); //记录日志
                ...
                case "edit_payment_fee": //修改支付手续费=================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AXEnums.ActionEnum.Edit.ToString(), "修改支付手续费，订单号:" + model.order_no); //记录日志
        ...
        #region 验证" + Resources.lang.username+ "是否可用==============================
        private void validate_username(HttpContext context)
        {
            ...
            //过滤注册" + Resources.lang.username+ "字符
            string[] strArray = userConfig.regkeywords.Split(',');
            foreach (string s in strArray)
            {
                if (s.ToLower() == user_name.ToLower())
                {
                    context.Response.Write("{ \"info\":\"" + Resources.lang.username+ "不可用\", \"status\":\"n\" }");
                    return;
                }
            }
            ...
            {
                context.Response.Write("{ \"info\":\"" + Resources.lang.username+ "已重复\", \"status\":\"n\" }");
                return;
            }
            context.Response.Write("{ \"info\":\"" + Resources.lang.username+ "可用\", \"status\":\"y\" }");
            return;
        }
        #endregion
        ...
        private void sms_message_post(HttpContext context)
        {
            ...
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"尚未登录或已超时，请登录后" + Resources.lang.operating+ "！\"}");
                return;
            }
            string mobiles = AXRequest.GetFormString("mobiles");
            string content = AXRequest.GetFormString("content");
            if (mobiles == "")
            {
        ...
        private int GetIsLoginAndIsStaticstatus()
        {
            ...
            if (adminInfo == null)
                return -1;
            else if (!new BLL.manager_role().Exists(adminInfo.role_id, "app_builder_html", AXEnums.ActionEnum.Build.ToString()))
                return -2;
