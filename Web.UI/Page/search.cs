...
namespace Web.UI.Page
{
    public partial class search : Web.UI.BasePage
    {
        ...
        protected override void ShowPage()
        {
            ...
            keyword = AXRequest.GetQueryStringValue("keyword");
            channel_name = AXRequest.GetQueryStringValue("channel_name");
        }
        ...
        protected DataTable get_search_list(int _pagesize, out int _totalcount)
        {
            ...
            //组合查询条件
            string strWhere = "(a.title like '%" + keyword + "%' or a.Barcode like '%" + keyword + "%') " + strwhere;
            ...
            dt.Columns.Add("stock_quantity", Type.GetType("System.Int32"));
            dt.Columns.Add("sell_price", Type.GetType("System.Decimal"));
            dt.Columns.Add("Barcode", Type.GetType("System.String"));
            dt.Columns.Add("id", Type.GetType("System.Int32"));
            DataSet ds = new BLL.article().GetGoodList(_pagesize, page, strWhere, "add_time desc,id desc", out _totalcount);
            if (ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (!string.IsNullOrEmpty(link_url))
                    {
                        ...
                        dr["id"] = dr1["id"]; //库存
                        dr["stock_quantity"] = dr1["stock_quantity"]; //库存
                        dr["sell_price"] = dr1["sell_price"]; //条形码
                        dr["Barcode"] = dr1["Barcode"]; //价格
