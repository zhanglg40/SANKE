<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="order_edit.aspx.cs" Inherits="Web.admin.order.order_edit" ValidateRequest="false" %>
...
<style type="text/css">
    .btnadd{ background:#cc; border:none; color:#666; cursor:pointer; font-family:"Microsoft Yahei"; font-size:12px; height:25px; line-height:25px; padding:0 12px; }
	.btnadd:hover{ color:#fff; background:#FF9C30; }
</style>
<script type="text/javascript">
    $(function () {
        $("#btnConfirm").click(function () { OrderConfirm(); });   //确认订单
        $("#btnPayment").click(function () { OrderPayment(); });   //确认付款
        $("#btnExpress").click(function () { OrderExpress(); });   //确认送货
    ...
    function DeleteOrder(ID) {
        var dialog = $.dialog.confirm('確認要刪除該項商品嗎？', function () {
            var postData = { "order_no": $("#spanOrderNo").text(), "id": ID, "edit_type": "order_delete" };
            //发送AJAX请求
    ...
    function EditOrder(ID) {
        var dialog = $.dialog({
            title: '修改商品數量',
            content: 'url:dialog/dialog_edit.aspx?id='+ID,
            min: false,
            max: false,
            lock: true,
            width: 550,
    ...
    function AddOrder() {
        var dialog = $.dialog({
            title: '新添商品',
            content: 'url:dialog/dialog_list.aspx?channel_id=2&category_id=0&article_list_view=Txt',
            min: false,
            max: false,
            lock: true,
            width: 800,
...
//        var dialog = $.dialog.confirm('确认订单后将无法修改金额，确认要继续吗？', function () {
            ...
            //发送AJAX请求
            sendAjaxUrl("", postData, "../../tools/admin_ajax.ashx?action=edit_order_status");
            this.window.close();
            return false;
//        });
    }
...
<div class="tab-content">
    ...
    <dd style="margin-left:50px;text-align:center;">
      <div class="order-flow" style="width:560px">
        ...
        <%if (model.express_status == 1)
          { %>
        <div class="order-flow-wait">
          <a class="order-flow-input">送货</a>
          <span><p class="name">等待送货</p></span>
        </div>
        <%}
  ...
  <HeaderTemplate>
  <dl>
    <dt><asp:Literal runat="server" Text="<%$ Resources:lang,goodlist%>" /></dt>
    <dd>
      <div>
        <%--<asp:Button id="btnAddOrder" runat="server" visible="true" class="btnadd" Text="新增商品" onclick="btnAddOrder_Click" />--%>
        <a href="javascript:void(0)" onclick="AddOrder()">新增商品</a>
      </div>
      ...
      <%--      <th width="12%">积分合计</th>--%>
        ...
        <%--    <td><%#Eval("real_price")%></td>
         ...
         <%--   <td><%#Convert.ToInt32(Eval("point")) * Convert.ToInt32(Eval("quantity"))%></td>--%>
            <td>
                <a href="javascript:void(0)" onclick="EditOrder('<%#Eval("id")%>')">修改</a>
                <a href="javascript:void(0)" onclick="DeleteOrder('<%#Eval("id")%>')">刪除</a>
            </td>
          </tr>
          </ItemTemplate>
          <FooterTemplate>
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="border-table" width="98%">
        <tr>
          ...
          <td>
           ...
           <tr>
          <th>邮编</th>
          <td><span id="spanPost"><%=model.post_code %></span> </td>
        </tr>
        <tr>
          <th>托运地址</th>
          <td><span id="spanArea"><%=model.city %></span> <span id="spanAddress"><%=model.address %></span></td>
        </tr>
           <tr>
          <th>手机</th>
        ...
        <tr>
          <th>商店号</th>
          <td><span id="spancompanyid"><%=model.companyid %></span></td>
        </tr>
        
        <tr>
          <th>税号</th>
          <td><span id="taxid"><%=model.taxid %></span></td>
        </tr>
         <tr>
          <th>公司名称</th>
          <td><span id="spancompanyname"><%=model.companyname %></span></td>
        </tr>
        <tr>
          <th>公司地址</th>
          <td><span id="spancompanyaddress"><%=model.companyaddress %></span></td>
        </tr>
      </table>
    </dd>
  </dl>
  ...
  <dl>
    ...
    <dd>
      <table border="0" cellspacing="0" cellpadding="0" class="border-table" width="98%">
        <tr>
          <th width="20%">支付方式</th>
          <td><%=new BLL.payment().GetTitle(model.payment_id) %></td>
        </tr>
       <%-- <tr>
          <th>配送小区</th>
          <td><%=new BLL.express().GetTitle(model.express_id) %></td>
        </tr>--%>
        <tr>
          <th>用户留言</th>
          <td><%=model.message %></td>
    ...
    <dd>
      <table border="0" cellspacing="0" cellpadding="0" class="border-table" width="98%">
        <tr>
          ...
          <td>
            <div class="position">
              <span id="spanRealAmountValue"><%=model.real_amount %></span> PLN
        ...
        <tr>
          ...
          <td>
            <div class="position">
              <span id="spanExpressFeeValue"><%=model.express_fee %></span> PLN
        ...
        <tr>
          ...
          <td>
            <div class="position">
              <span id="spanPaymentFeeValue"><%=model.payment_fee %></span> PLN
...
<div class="page-footer">
  <div class="btn-list">
    ...
    <input id="btnExpress" runat="server" visible="false" type="button" value="确认送货" class="btn" />
